...
def test_grade_assignment_invalid_grade(client, h_principal):
    ...
    assert response.json["message"] == {
        "data": {"state": "GRADED"},
        "message": "{'grade': ['Invalid enum member Z']}",
    }
...
def test_grade_assignment_no_grade(client, h_principal):
    ...
    """
    response = client.post(
        "/principal/assignments/grade", json={"id": 1}, headers=h_principal
    )

    assert response.status_code == 400
    assert response.json["message"] == {
        "data": {"state": "GRADED"},
        "message": "{'grade': ['Missing data for required field.']}",
    }
...
def test_grade_non_existent_assignment(client, h_principal):
    ...
    response = client.post(
        "/principal/assignments/grade",
        json={"id": 100, "grade": GradeEnum.A.value},
        headers=h_principal,
    )
...
def test_no_header(client):
    ...
    h_principal = {"X-Principal": json.dumps({"principal_id": 3, "user_id": 5})}
