...
def test_grade_assignment_bad_assignment(client, h_teacher_1):
    ...
    response = client.post(
        "/teacher/assignments/grade", headers=h_teacher_1, json={"id": 100000, "grade": "A"}
    )
...
def test_valid_grade_assignment_draft_assignment(client, h_teacher_2):
    response = client.post("/teacher/assignments/grade", headers=h_teacher_2, json={"id": 4, "grade": "D"})
    assert response.status_code == 200


def test_teacher_repr():
    teacher = Teacher(id=789)
    result = repr(teacher)
    assert result == "<Teacher 789>"
