...
def test_get_assignments_no_assignments_student_1(client, h_student_1):
    ...
    db.engine.execute(
        text(
            "INSERT INTO USERS (id, username, email, created_at, updated_at) VALUES (:id, :name, :email, :created_at, :updated_at)"
        ),
        {
            "id": 7,
            "name": "Test User 3",
            "email": "testuser@fyle.com",
...
def test_submit_assignment_invalid_teacher(client, h_student_2):
    ...
    response = client.post(
        "/student/assignments/submit",
        headers=h_student_2,
        json={"id": 4, "teacher_id": 999},  # Assuming teacher ID 999 doesn't exist
    )
...
def test_upsert_assignment_wrong_student(client, h_student_1):
    ...
    response = client.post(
        "/student/assignments",
        headers=h_student_1,
        json={"content": "New Content", "teacher_id": 2, "id": 3},
    )
...
def test_submit_assignment_invalid_request_body(client, h_student_1):
    ...
    response = client.post(
        "/student/assignments/submit", headers=h_student_1, json={"teacher_id": 1}
    )
