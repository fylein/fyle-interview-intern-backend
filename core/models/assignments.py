...
class Assignment(db.Model):
    ...
    @classmethod
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal, isTeacher: bool):
        assignment = Assignment.get_by_id(_id)
        assertions.assert_found(assignment, 'No assignment with this id was found')
        if isTeacher:
            #The teacher making the request and the teacher who was suppose to grade the assignment with that specific id should match
            ...
            #Assignments can be graded by principal if it is either 'SUBMITTED' or 'GRADED'
    ...
    def get_all_submitted_and_graded_assignments(cls):
        return cls.filter(cls.state == AssignmentStateEnum.SUBMITTED, cls.grade != None).all()