...
class Assignment(db.Model):
    ...
    @classmethod
    def grade_student(cls, _id, grade, principal: Principal):
        assignment = Assignment.get_by_id(_id)
        gradeSet = set(item.value for item in GradeEnum)
        assertions.assert_found(assignment, 'No assignment with this id was found')
        assertions.assert_valid(assignment.teacher_id == principal.teacher_id, 'This assignment was not submitted(at least to this teacher)')
        assertions.assert_valid(assignment.content is not None, 'assignment with empty content cannot be graded')
        assertions.assert_validate(grade in gradeSet, 'Invalid Grade')
        assertions.assert_integrity(assignment.state!=AssignmentStateEnum.GRADED, "Already Graded")
