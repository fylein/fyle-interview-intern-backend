...
class Assignment(db.Model):
    ...
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal):
        ...
        if auth_principal.principal_id is not None:
            assertions.assert_valid(assignment.state != 'DRAFT', 'Assignments in Draft state cannot be graded by principal')
    ...
    def get_assignments_by_teacher(cls, teacher_id):
        return cls.filter(cls.teacher_id == teacher_id, cls.state.in_(['SUBMITTED', 'GRADED'])).all()
    
    @classmethod
    def get_assignments_by_principal(cls):
        return cls.filter(cls.state.in_(['SUBMITTED', 'GRADED'])).all()
