...
def get_all_teachers():
    ...
    with db.session() as session:
        ...
        teachers_data = [{'id': teacher.id, 'user_id': teacher.user_id,
                          'created_at': teacher.created_at.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3], 'updated_at': teacher.updated_at.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3]} for teacher in teachers]
